  ...
  methods: {
    ...
    saveData() {
      courseAPI.saveCourseInfo(this.courseInfo).then(response => {
        ...
        this.$parent.courseId = response.data.courseId// 获取id
        this.$parent.active = 1 // 改变父组件的成员
      })
    },
    // 上传成功回调
    ...
    fetchCourseInfoById(id) {
      courseAPI.getCourseInfoById(id).then(response => {
        ...
        // 填充二级列表
        this.subjectList.forEach(response => {
          if (response.id === this.courseInfo.subjectId) {
            this.subjectLevelTwoList = response.children
          }
        })
      })
    },
